...
function icit_srdb_replacer( $connection, $search = '', $replace = '', $tables = array( ) ) {
	...
	if ( is_array( $tables ) && ! empty( $tables ) ) {
		foreach( $tables as $table ) {
			$report[ 'tables' ]++;
                        $html .= "<li>(". $count++ .") Table: <strong>" . $table . "</strong></br>";
			$columns = array( );
				...
				while ( $row = mysql_fetch_array( $data ) ) {
					...
					foreach( $columns as $column => $primary_key ) {
						...
						if ( $edited_data != $data_to_fix ) {
							...
							$upd = true;
                                                        $noupdate = false;
                                                        $html .= "---> Column Name: <strong>" . $column . "</strong> - ";
						}
					...
					if ( $upd && ! empty( $where_sql ) ) {
						...
						if ( ! $result ){
							$report[ 'errors' ][] = mysql_error( );
                                                        $html .= "Row Id: " . $row["ID"] . " (update error)</br>";
                                                        $html .= "</li>";
                                                }
						else
                                                    {
							$report[ 'updates' ]++;
                                                        $html .= "Row Id: <strong>" . $row["ID"] . "</strong> (update success)</br>";
                                                        $html .= "</li>";
                                                }
...
<body>
	...
	<h1>Safe Search Replace</h1>
        <a href="https://interconnectit.com/products/search-and-replace-for-wordpress-databases/"><p class="byline">by interconnect/<strong>it</strong></p></a>
	<?php
/*
...
switch ( $step ) {
	case 1:
		...
		<h2>Load DB connection values from WordPress?</h2>
                ...
                <ul class="warning-message">
                    <li>Take a backup first, and carefully test the results of this code!</li>
                    <li>Don't Forget to Remove Me!</li>
                    <li>Use Of This Script Is Entirely At Your Own Risk</li>
                    <li>There is NO UNDO here!</li>
                </ul>
	...
	case 2:
		...
		<h2>Database details - (Required: keep it as default)</h2>
		<form action="<?php icit_srdb_form_action( ); ?>" method="post">
			<fieldset>
				<p>
	...
	case 3:
		...
		<h2>Which tables do you want to scan?</h2>
                <h2>PLEASE!</h2>
                <ul class="warning-message">
                    <li>- Be carefully! There is no UNDO</li>
                    <li>- Never select all tables</li>
                    <li>- Select tables that you need to replace strings. Ex: wp_links, wp_posts, wp_options</li>
			...
			<fieldset>
				...
				<p>
					...
					foreach( $all_tables as $table ) {
						echo '<option value="' . esc_html_attr( $table ) . '">' . $table . '</option>';
					} ?>
					</select>
