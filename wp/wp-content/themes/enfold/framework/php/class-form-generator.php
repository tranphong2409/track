...
if( ! class_exists( 'avia_form' ) )
{
	class avia_form
	{
		...
		function text($id, $element)
		{
			...
			$form_el = ' <input name="'.$id.'" class="text_input '.$element_class.'" type="'.$type.'" id="'.$id.'" placeholder="'.$element['label'].$required.'" value="'.$value.'"/>';
			$label = '';
		...
		function textarea($id, $element)
		{
			...
			$this->elements_html .= '	 <textarea name="'.$id.'" class="text_area '.$element_class.'" placeholder="'.$element['label'].$required.'" cols="40" rows="7" id="'.$id.'" >'.$value.'</textarea>';
			$this->elements_html .= "</p>";
		}
